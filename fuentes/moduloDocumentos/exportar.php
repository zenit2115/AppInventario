...
$con=@mysqli_connect('localhost', 'root', '123456','inventario');
 ...
 $sql = "SELECT * FROM ict_tipodocumentos ORDER BY 1 ASC";
 ...
 if ($registros > 0) {
   ...
   $objPHPExcel->
    ...
    getProperties()
        ...
        ->setTitle("Exportar tipo documento")

        ->setSubject("Ejemplo 1")

        ->setDescription("Documento generado...")

        ->setKeywords("documento")

        ->setCategory("documento");    



		$objPHPExcel->getActiveSheet()->setCellValue('A1','Detalle tipo Documento');

		

      $objPHPExcel->setActiveSheetIndex()->setCellValue('A3', 'NOMBRE TIPO DOCUMENTO');	  
...
while($row = mysqli_fetch_array($resultado)){
      ...
      $objPHPExcel->setActiveSheetIndex()->setCellValue('A'.$i, $row['icp_id_documentos']);

	  $objPHPExcel->setActiveSheetIndex()->setCellValue('B'.$i, $row['ictipodocumento']);
